...
import tkMessageBox
import random
from frame_animations import animations, movements, movChoregrapheExp 
from glaskanie_miro import namesMiro, timesMiro, keysMiro
...
class App:
    def __init__(self, window, window_title):
        ...
        #Language at the start set up to PL
        ...
        #top right frame (consists of 'IP' and 'On/Off' butttons)
        ...
        #bottom frame (consists of: collumn right, collumn middle, collumn left )
        ...
        #scrollBar = tk.Scrollbar(leftColButtons, highlightbackground =self.bgColor, troughcolor =self.bgColor, bd = 0, command=leftColButtons.yview)
        #scrollBar.pack(side=tk.RIGHT, padx = (10,0), expand = True, fill = tk.Y)
        #leftColButtons.configure(yscrollcommand=scrollBar.set)
    ...
    def langPL(self):
        global LANGUAGE
        if LANGUAGE == "English":
            self.PLBorder.configure(bg = "#64a225")
            self.ENGBorder.configure(bg = "white")
            LANGUAGE = "Polish"
            setLanguage(LANGUAGE)
            print("Language changed to " + LANGUAGE)
    ...
    def langENG(self):
        global LANGUAGE
        if LANGUAGE == "Polish":
            self.ENGBorder.configure(bg = "#64a225")
            self.PLBorder.configure(bg = "white")
            LANGUAGE = "English"
            setLanguage(LANGUAGE)
            print("Language changed to " + LANGUAGE)
    ...
    def sayTextBox(self,e ):
        rawText = self.textFieldFbF.get('1.0', tk.END)
    ...
    def executeMovement(self, movementTag):
        ...
        # movChoregrapheExp[][] - first idex coresponds to movent tag (indexes 0-6)
        # - second idex coresponds to names, times, or keys array (indexes 0-2)
        for i in range(6):
            if movements[i] == movementTag:
                names = movChoregrapheExp[i][0]
                times = movChoregrapheExp[i][1]
                keys = movChoregrapheExp[i][2]
